...
class PromptsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create prompt" do
    assert_difference("Prompt.count") do
      post prompts_url, params: { prompt: { level: @prompt.level, topic: @prompt.topic, type: @prompt.type } }
    end
  ...
  test "should update prompt" do
    patch prompt_url(@prompt), params: { prompt: { level: @prompt.level, topic: @prompt.topic, type: @prompt.type } }
